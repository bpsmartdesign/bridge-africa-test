    ...
    <b-colxx xxs="12" md="10" class="mx-auto my-auto">
        <b-card class="auth-card" no-body>
            ...
            <div class="form-side">
                ...
                <b-form @submit.prevent="login" class="av-tooltip tooltip-label-bottom">
                    ...
                    <div class="d-flex justify-content-between align-items-center">
                        <router-link tag="a" to="/user/register">Create an Account</router-link>.
...
export default {
    ...
    methods: {
        login() {
            ...
            setTimeout(() => {
                const user = this.allUsers.find(u => u.email === this.form.email && u.password === this.form.password)

                console.log(user, this.allUsers)
                
                if (user === undefined) {
                    this.loginError = true
                    this.$notify("error", "Login Error", "Username or Password error, or there is not any user account yet, Go register first", {
                        duration: 3000,
                        permanent: false
                    });
                } else {
